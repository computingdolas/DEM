...
//Output file writer
void VTKWriter::writeVTKOutput(const cudaDeviceBuffer<real_d> &mass, const cudaDeviceBuffer<real_d> &radius, const cudaDeviceBuffer<real_d> &position, \
                               const cudaDeviceBuffer<real_d> &velocity, const cudaDeviceBuffer<real_d> &rotvec, int num_particles){
    ...
    //Output rotation
    outfile<<"VECTORS q double"<<std::endl;
    for(int i=0;i<num_particles;i++){
        outfile<<std::setprecision(8)<<rotvec[i*3]<<" "<<rotvec[i*3+1]<<" "<<rotvec[i*3+2]<<std::endl;
    }
}
